...
_DATA_URLS = {
    ...
    # },
    "all": {
        "train": [f"/fs/cml-datasets/Pile/train/{i:0>2}.jsonl.zst" for i in range(30)],
        "validation": [f"/fs/cml-datasets/Pile/val.jsonl.zst"],
        "test": [f"/fs/cml-datasets/Pile/test.jsonl.zst"],
    },
    "all_train": {
        "train": [f"/fs/cml-datasets/Pile/train/{i:0>2}.jsonl.zst" for i in range(30)],
    },
    "all_train_00": {
        "train": [f"/fs/cml-datasets/Pile/train/00.jsonl.zst"],
    },
    "all_train_10": {
        "train": [f"/fs/cml-datasets/Pile/train/10.jsonl.zst"],
    },
    "all_train_00-04": {
        "train": [f"/fs/cml-datasets/Pile/train/{i:0>2}.jsonl.zst" for i in range(5)],
    },
    "all_val": {
        "validation": [f"/fs/cml-datasets/Pile/val.jsonl.zst"],
    },
...
_FEATURES = {
    ...
    "all_train": datasets.Features(
        {
            "text": datasets.Value("string"),
            "meta": {"pile_set_name": datasets.Value("string")},
        }
    ),
    "all_train_00": datasets.Features(
        {
            "text": datasets.Value("string"),
            "meta": {"pile_set_name": datasets.Value("string")},
        }
    ),
    "all_train_00-04": datasets.Features(
        {
            "text": datasets.Value("string"),
            "meta": {"pile_set_name": datasets.Value("string")},
        }
    ),
    "all_val": datasets.Features(
        {
            "text": datasets.Value("string"),
            "meta": {"pile_set_name": datasets.Value("string")},
        }
    ),
    "all_test": datasets.Features(
        {
            "text": datasets.Value("string"),
            "meta": {"pile_set_name": datasets.Value("string")},
        }
    ),
    "all_val_test": datasets.Features(
        {
            "text": datasets.Value("string"),
            "meta": {"pile_set_name": datasets.Value("string")},
        }
...
class ThePile(datasets.GeneratorBasedBuilder):
    ...
    def _split_generators(self, dl_manager):
        ...
        if self.config.name.startswith("all"):
            ...
            print("dl_manager.dataset_name: ", dl_manager._dataset_name)
            print("dl_manager.data_dir: ", dl_manager._data_dir)
            print("dl_manager.download_config: ", dl_manager.download_config)
            print("dl_manager.base_path: ", dl_manager._base_path)
            data_dir = dl_manager.download(_DATA_URLS[self.config.name])
            print("data_dir: ", data_dir)

            split_list = [] # [datasets.Split.TRAIN, datasets.Split.VALIDATION, datasets.Split.TEST]
            if "train" in self.config.name: split_list.append(datasets.Split.TRAIN)
            if "val" in self.config.name: split_list.append(datasets.Split.VALIDATION)
            if "test" in self.config.name: split_list.append(datasets.Split.TEST)
